...
//            map.put("callback_url", XYBankConfig.return_url);
//            map.put("callback_url", "http://qiuguojie.wicp.net/success.html?orderNum=DD2742315656");
            //app支付(sdk)
     ...
     * @return
     */
    public static Map<String, String> qqPay(String body, String orderid, String money, String ip
        ...
        try {
            ...
            map.put("service", XYBankConfig.WFTmethod_QQ);

            String sign = signMd5(key, map);
            map.put("sign", sign);
    		SortedMap<String, String> xmlMap = new TreeMap<String, String>(map);
    		String xmlInfo = XmlUtils.parseXML(xmlMap);
    		log.info("=====>提交参数转换为XML格式：\n" + xmlInfo);
			...
			System.out.println("=====>转换为map值为：" + toxmlMap);
			if ("0".equals(toxmlMap.get("status")) && "0".equals(toxmlMap.get("result_code"))) {
				returnMap.put("qr_code", toxmlMap.get("code_url"));
				returnMap.put("code","10000");
				returnMap.put("out_trade_no",orderid);
				log.info("=====>兴业银行QQ威富通扫码支付调起成功:qr_code : " + toxmlMap.get("code_url"));
				System.out.println("=====>兴业银行QQ威富通扫码支付调起成功:qr_code : " + toxmlMap.get("code_url"));
			} else {
				returnMap.put("code","10001");
				returnMap.put("msg", info);
				log.error("=====>兴业银行QQ威富通扫码支付失败原因:" + info);
				System.out.println("=====>兴业银行QQ威富通扫码支付失败原因:" + info);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		qqPay("测试支付", System.currentTimeMillis()+"", ""+DecimalUtil.yuanToCents("0.01"), "192.168.1.172"
    			, "101550068466", "37b559c8d3e45cb17e2d9dbc3cfcbc93");
