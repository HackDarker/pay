package com.pay.business.util.suiningYH;
...
public class SNBankPay {
	private static Logger log = Logger.getLogger(SNBankPay.class);
	 ...
	 * @param key
	 */
	public static Map<String,String> snWXGzhPay(String body, String orderid, String money, String ip, String openId, String appId, String mchId, String key) throws Exception{
		...
		//接口类型
        sginMap.put("service", SNBankConfig.WFT_GZH);
        //商户ID
        sginMap.put("mch_id",mchId);
        //异步回调URL
        sginMap.put("notify_url", SNBankConfig.notify_url);
        //
        sginMap.put("callback_url", SwiftpassConfig.CALLBACK_URL);
        //appid
        sginMap.put("sub_appid", appId);
        sginMap.put("is_raw", "1");
		...
		try {
			String info = sendPost(SNBankConfig.WFT_url, xmlInfo);
			log.info("=====>遂宁银行公众号返回的预支付订单信息:\n" + info + " \n");
			System.out.println("=====>遂宁银行公众号返回的预支付订单信息:\n" + info + " \n");
			...
			if (!SignUtils.checkParam(resultMap,key)) {
                ...
                log.error("=====>遂宁银行公众号支付调起失败原因：验证签名不通过");
            } else {
                if ("0".equals(resultMap.get("status")) && "0".equals(resultMap.get("result_code"))) {
                	//组装可以访问的URL
                	String url=SwiftpassConfig.WFT_PAY_URL+resultMap.get("token_id");
                	if(resultMap.containsKey("pay_info")){
                		JSONObject  jasonObject = JSONObject.parseObject(resultMap.get("pay_info"));
                		Map<String,String> jsMap = (Map)jasonObject;
                		resultMap.put("jsapi_appid", jsMap.get("appId"));
						resultMap.put("jsapi_timestamp",jsMap.get("timeStamp"));
						resultMap.put("jsapi_noncestr", jsMap.get("nonceStr"));
						resultMap.put("jsapi_package", jsMap.get("package"));
						resultMap.put("jsapi_signtype", jsMap.get("signType"));
						resultMap.put("jsapi_paysign", jsMap.get("paySign"));
                	}else{
                		resultMap.put("pay_url",url);
                	}
                	resultMap.put("code","10000");
                    log.info("=====>遂宁银行公众号支付调起成功!访问的URL为："+url);
                }else{
                	resultMap.put("code","10001");
                	resultMap.put("msg","失败原因："+resultMap+"(上游问题)");
                	 log.error("=====>遂宁银行公众号支付调起失败原因："+resultMap);
                } 
            }
		} catch (Exception e) {
			log.error("=====>遂宁银行公众号支付操作失败，原因：", e);
			resultMap.put("code","10001");
     ...
     * @return
     */
    public static Map<String, String> snWFTWXWapPay(String body, String orderid, String money, String ip
    		,String appType,String mchId,String key,String dictNameLike) {
        ...
        try {
            ...
            // 异步通知地址
            map.put("notify_url", SNBankConfig.notify_url);
            //app支付(sdk)
            if("APP".equals(dictNameLike)){
            	// 接口类型
            ...
            Map<String, String> resultMap = com.pay.business.util.wftpay.XmlUtils.post(SNBankConfig.WFT_url, map);
            ...
            if(resultMap.containsKey("sign")){
                if(!SignUtils.checkParam(resultMap, key)){
                	commonErrorMap.put(ERROR_KEY, resultMap.toString());
                    ...
                    if("0".equals(resultMap.get("status"))){
                        ...
                        if("APP".equals(dictNameLike)){
                        	...
                        	if(resultMap.containsKey("token_id")){
                        		String token_id = resultMap.get("token_id");
                        		returnMap.put("token_id", token_id);
                        	}
                        	if(resultMap.containsKey("services")){
                        		 String services = resultMap.get("services");
                        		 returnMap.put("services", services);
                        	}
                    	...
                    	commonErrorMap.put(ERROR_KEY, resultMap.toString()+"(上游问题)");
                    	return commonErrorMap;
                    }
                }
            } else{
	 ...
	 * @return
	 */
	public static Map<String, String> snWFTWXScanPay(String out_trade_no,String total_fee, String body, String ip, String mch_id, String key) {
		Map<String, String> resultMap = new HashMap<String, String>();
		//随机字符串
		...
		// 签名
		Map<String, String> sginMap = new HashMap<String, String>();
		sginMap.put("service", SNBankConfig.WFTmethod_WX);
		...
		try {
			String info = sendPost(SNBankConfig.WFT_url, xmlInfo);
			log.info("=====>遂宁银行威富通返回的预支付订单信息:\n" + info + " \n");
			System.out.println("=====>遂宁银行威富通返回的预支付订单信息:\n" + info + " \n");
			...
			if ("0".equals(toxmlMap.get("status")) && "0".equals(toxmlMap.get("result_code"))) {
				...
				log.info("=====>遂宁银行支付宝威富通扫码支付调起成功:qr_code : " + toxmlMap.get("code_url"));
				System.out.println("=====>遂宁银行支付宝威富通扫码支付调起成功:qr_code : " + toxmlMap.get("code_url"));
			} else {
				resultMap.put("code","10001");
				resultMap.put("msg", info);
				log.error("=====>遂宁银行支付宝威富通扫码支付失败原因:" + info);
				System.out.println("=====>遂宁银行支付宝威富通扫码支付失败原因:" + info);
			}
		} catch (Exception e) {
			log.error("=====>遂宁银行支付宝威富通扫码支付操作失败，原因：", e);
			resultMap.put("code","10001");
			resultMap.put("msg", e.toString());
		}
		return resultMap;
	 ...
	 * @return
	 */
	public static Map<String, String> snWFTAliaScanPay(String out_trade_no,String total_fee, String body, String ip, String mch_id, String key) {
		Map<String, String> resultMap = new HashMap<String, String>();
		//随机字符串
		String nonce_str=new Date().getTime()+ RandomUtil.generateString(4);
		//签名
		...
		// 签名
		Map<String, String> sginMap = new HashMap<String, String>();
		sginMap.put("service", SNBankConfig.WFTmethod_ZFB);	//接口类型
		sginMap.put("mch_id", mch_id);
		sginMap.put("nonce_str", nonce_str);
		sginMap.put("body", body);
		sginMap.put("mch_create_ip", ip);
	 ...
	 * @return
	 */
	public static Map<String, String> snWFTQQScanPay(String out_trade_no,String total_fee, String body, String ip, String mch_id, String key) {
		Map<String, String> resultMap = new HashMap<String, String>();
		//随机字符串
		String nonce_str=new Date().getTime()+ RandomUtil.generateString(4);
		//签名
		...
		// 签名
		Map<String, String> sginMap = new HashMap<String, String>();
		sginMap.put("service", SNBankConfig.WFTmethod_QQ);	//接口类型
		sginMap.put("mch_id", mch_id);
		sginMap.put("nonce_str", nonce_str);
		sginMap.put("body", body);
		sginMap.put("mch_create_ip", ip);
	 ...
	 * 遂宁银行账单下载
	 */
	public static String [] snOderFind(String billDate,String mchId,String key){
		try {
            // 构造下单所需要的参数
            SortedMap<String, String> map = new TreeMap<String, String>();
            Collections.synchronizedSortedMap(map);
            ...
            // 接口类型
            map.put("service", SNBankConfig.DOWNLOAD_SERVICE);
            
            // 账单日期
            map.put("bill_date", billDate);
            
            // 账单类型
            map.put("bill_type", SNBankConfig.BILL_TYPE);
            ...
            String [] str = downPost(SNBankConfig.DOWNLOAD_URL, map);
            return str;
        } catch (Exception e) {
        	e.printStackTrace();
        }
    ...
    public static Map<String,String> refund(String orderid,String refundOrderNum, String money
        ...
        try {
            ...
            // 接口类型
            map.put("service", SNBankConfig.REFUND_SERVICE);
    ...
    public static Map<String,String> queryOrder(String orderid,String mchId,String key){
    	...
    	try {
            ...
            // 接口类型
            map.put("service", SNBankConfig.QUERY_SERVICE);
	...
	public static void main(String[] args) {
		Map<String, String> xySZWFTAliaScanPay = snWFTAliaScanPay("DD20170912173655", "1", "DD20170912173655", "192.168.0.1", "129530003797", "8201dcd5a3850206ddd284d7bfa131ac");
		System.out.println(xySZWFTAliaScanPay.toString());
	...
	public static String signMd5(String key, Map<String, String> map) {
		...
		sgin = sgin + "&key=" + key;
		log.info("=====>遂宁银行支付宝扫码支付参数加密拼接为：" + sgin);
