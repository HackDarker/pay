...
public class Payv2PayOrderRefundServiceImpl extends BaseServiceImpl<Payv2PayOrderRefund, Payv2PayOrderRefundMapper> implements Payv2PayOrderRefundService {
	...
	public Map<String, Object> payRefund(Map<String, Object> map, Long appId, String appSecret) throws Exception {
		...
		if (scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ALIPAY_APP) 
			...
			if (scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ALIPAY_WEB)) {
				// 调用支付宝退款
...
//					int refundMoney2=(int)refundMoney1;
		 ...
		 */
				...
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_XYSZ_QQ_SCAN)
				...
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_ALI_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_WEB)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_SNBANK_ALI_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_SNBANK_QQ_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_WAP)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_GZH)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ZSXM_WEIXIN_SCAN)
				||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ZSXM_ALI_SCAN)
				) {
			// 生成退款订单号 RR+appId+orderNum
			orderRefund = createRefundOrder(orderRefund, payOrder, refundMoney, map, null);
			//兴业深圳退款
			if (PayRateDictValue.PAY_TYPE_XYSZ_ALI_SCAN.equals(scd.getDictName()) || PayRateDictValue.PAY_TYPE_XYSZ_WEIXIN_SCAN.equals(scd.getDictName())
					...
					|| PayRateDictValue.PAY_TYPE_XYSZ_QQ_SCAN.equals(scd.getDictName()) ) {
				if(ppwr.getRateKey1()!=null&&ppwr.getRateKey2()!=null){
					...
					logger.error("兴业深圳支付退款失败：数据库参数配置不完整-->RateKey1,RateKey2");
				}
			}
			
			//遂宁银行退款
			if (PayRateDictValue.PAY_TYPE_SNBANK_ALI_SCAN.equals(scd.getDictName()) 
					|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_SCAN.equals(scd.getDictName())
					|| PayRateDictValue.PAY_TYPE_SNBANK_QQ_SCAN.equals(scd.getDictName())
					|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_WAP.equals(scd.getDictName())
					|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_GZH.equals(scd.getDictName())) {
				if(ppwr.getRateKey1()!=null&&ppwr.getRateKey2()!=null){
					Map<String, String> refund = SNBankPay.refund(payOrder.getOrderNum(), orderRefund.getRefundNum(),
							DecimalUtil.yuanToCents("" + payOrder.getPayDiscountMoney()), "" + DecimalUtil.yuanToCents("" + refundMoney), ppwr.getRateKey1(),
							ppwr.getRateKey2());
					if (refund.containsKey("refund_fee")) {
						// 成功
					...
					logger.error("遂宁银行支付退款失败：数据库参数配置不完整-->RateKey1,RateKey2");
				}
			}
			//浦发深圳发起退款(异步通知结果)
			else if(scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_ALI_SCAN)
					||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_SCAN)
					||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_WEB)){
				Map<String, String> refund = PFSZBankPay.refund(orderRefund.getRefundNum(), payOrder.getRateKey1(), payOrder.getRateKey2(), 
						new SimpleDateFormat("yyyyMMdd").format(payOrder.getCreateTime()), payOrder.getOrderNum(), DecimalUtil.yuanToCents("" + refundMoney), 
						payOrder.getRateKey5(), payOrder.getRateKey6());
				if("10000".equals(refund.get("code"))){
					...
					logger.error("浦发深圳发起退款失败："+refund.toString());
					resultMap.put("error_code", "REFUND_FAILED"); // 退款失败
					resultMap.put("error_msg", "退款失败");
				}
			}
			//招行厦门发起退款
			else if(scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ZSXM_WEIXIN_SCAN)
					||scd.getDictName().equals(PayRateDictValue.PAY_TYPE_ZSXM_ALI_SCAN)){
				String payType="1";
				if(scd.getDictName().contains("_WEIXIN")){
					payType="2";
				}
				Map<String, String> refund = ZsxmPay.refund(payOrder.getOrderNum(),orderRefund.getRefundNum()
						,DecimalUtil.yuanToCents("" + payOrder.getPayDiscountMoney())
						,DecimalUtil.yuanToCents(""+refundMoney),payType
						, payOrder.getRateKey1(), payOrder.getRateKey2());
				if("10000".equals(refund.get("code"))){
					//orderRefund.setRefundStatus(3);
					orderRefund.setRefundStatus(1);// 退款成功
					orderRefund.setRefundTime(new Date());
			...
			if (scd.getDictName().equals(PayRateDictValue.PAY_TYPE_WFT_WEIXIN)) {
				if(ppwr.getRateKey1()!=null&&ppwr.getRateKey2()!=null){
					Map<String, String> weChatMap = WftWeChatPay.refund(payOrder.getOrderNum(), orderRefund.getRefundNum(),
							DecimalUtil.yuanToCents("" + orderRefund.getRefundMoney()), "" + DecimalUtil.yuanToCents("" + refundMoney), ppwr.getRateKey1(),
							ppwr.getRateKey2());
...
//					double refundMoney1 = refundMoney * 100;
	...
	public Double refundMoneySum(Map<String, Object> map) {
		return payv2PayOrderRefundMapper.refundMoneySum(map);
	}
}
