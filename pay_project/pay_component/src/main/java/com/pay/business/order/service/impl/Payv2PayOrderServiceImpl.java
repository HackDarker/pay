...
public class Payv2PayOrderServiceImpl extends BaseServiceImpl<Payv2PayOrder, Payv2PayOrderMapper> implements Payv2PayOrderService {
		...
		if (payOrder != null) {
			if (payOrder.getPayStatus() != PayFinalUtil.PAY_ORDER_SUCCESS && payOrder.getPayStatus() != 5) {
				...
				Payv2BussSupportPayWay pbspw = payv2BussSupportPayWayService.getWayRate(wayList
						, payOrder.getPayMoney(), payOrder.getCompanyId(),payWayId.intValue(),pbc.getIsRandom(),"APP");
	...
	public boolean aliPayCallBack(Map<String, String> params,Payv2PayOrder order) throws Exception {
		...
		String out_transaction_id = null;//第三方订单号
		if(params.containsKey("out_transaction_id")){
			out_transaction_id = params.get("out_transaction_id");
		}
...
//			order = payv2PayOrderMapper.selectSingle(order);
			if (order != null && order.getPayStatus() != PayFinalUtil.PAY_ORDER_SUCCESS) {
				...
				if(order.getAppId()!=null){
					...
					param.put("id", order.getCompanyId());
					pbc = payv2BussCompanyMapper.queryTokenById(param);
				}
				// 交易支付成功
				...
				order.setOutTransactionId(out_transaction_id);
				...
				// 通知商户
				return notifyMerchant(order, pbca,pbc);
				/*
	...
	public boolean dyAliPayCallBack(Map<String, String> params,Payv2PayOrder order) throws Exception {
		...
		if ("TRADE_SUCCESS".equals(trade_status)) {
			if (order != null && order.getPayStatus() != PayFinalUtil.PAY_ORDER_SUCCESS) {
				...
				// 通知商户
				return notifyMerchant(order, pbca,null);
			}
		} else {
			logger.debug("支付集-支付宝回调暂不处理的交易状态:" + trade_status);
		}
	 ...
	 * @throws Exception
	 */
	private boolean notifyMerchant(Payv2PayOrder order, Payv2BussCompanyApp pbca,Payv2BussCompany pbc) throws Exception {
		...
		if(TJRedisValue==null){
			...
			if(pbca==null){
				String token = pbc.getToken();
				System.out.println("商户手机token:"+token);
				if(StringUtils.isNoneBlank(token)){
					if(token.length()<64){//安卓推送
						System.out.println("安卓用户："+token+"推送中");
						PushDataUtil.asyncPoolAndroidPush(null, "收款通知", "您有一笔新订单！到账"+order.getPayMoney()+"元。", token,2);
					}else{//IOS推送
						System.out.println("苹果用户："+token+"推送中");
						List<String> list = new ArrayList<String>();
						list.add(token);
						PushDataUtil.asyncPoolIosPush(null, list, "您有一笔新订单！到账"+order.getPayMoney()+"元。");
					}
				}
			}
		...
		//D0通道  交易额叠加
		if(order.getWayArrivalType()!=null&&order.getWayArrivalType()==3
				&&order.getWayArrivalValue()!=null&&order.getWayArrivalValue()==0){
			//将提现订单号存入redis:以防重复统计
			String TXRedis="TX"+order.getOrderNum();
			String TXRedisValue=RedisDBUtil.redisDao.getString(TXRedis);
			if(TXRedisValue==null||TXRedisValue.equals("")){
				Date d = order.getCreateTime();
				int h = d.getHours();
				//--------------每天22点之后和8点之前不记录------------
				TxPayRateBalance txPayRateBalance = new TxPayRateBalance();
				txPayRateBalance.setCompanyId(companyId);
				txPayRateBalance.setRateId(rateId);
				txPayRateBalance.setCreateTime(new Date());
				txPayRateBalance.setBalance(order.getPayMoney());
				if(h>=8&&h<22){
					txPayRateBalance.setTxBalance(order.getPayMoney());
					//如果不存在新增，存在更新
					System.out.println("8-22点D0通道！记录可提交易额叠加金额："+(order.getPayMoney()));
					//----------------------------------------------------end--------------------------------------------------//
				}else if(h>=22){
					txPayRateBalance.setT1Balance(order.getPayMoney());
					System.out.println("22-24点D0通道！不在记录时间范围中！22点-24点可提现交易额不叠加金额,叠加T1交易额字段");
				}else{
					txPayRateBalance.setTxBalance(order.getPayMoney());
					//如果不存在新增，存在更新
					System.out.println("0-8点D0通道！记录可提交易额叠加金额："+(order.getPayMoney()));
				}
				txPayRateBalanceMapper.insertOrUpdate(txPayRateBalance);
				RedisDBUtil.redisDao.setString(TXRedis,TXRedis,86400);
			}
		}
		...
		if(pbca!=null){
			...
			param.put("sign", sign);
			notifyUrl=order.getNotifyUrl() == null ? pbca.getCallbackUrl() : order.getNotifyUrl();
		}else{//线下扫码
			...
			if(!StringUtils.isNoneBlank(notifyUrl)){
				...
				logger.info("========》回调地址不存在,直接修改订单状态成功");
				return true;
			}
			// 参数签名
			String sign = PaySignUtil.getSign(param, pbc.getCompanySecret());
			param.put("sign", sign);
		}
		
		final Payv2PayOrder payOrder = order;
		new Thread(new Runnable() {
			public void run() {
				//先修改支付状态
				payOrder.setCallbackTime(new Date());
				payOrder.setPayStatus(PayFinalUtil.PAY_ORDER_SUCCESS_BACKFAIL); // 支付成功回调失败
				payv2PayOrderMapper.updateByEntity(payOrder);
			}
		}).start();
		...
		//新接入商户回调方式
		if((pbca!=null&&pbca.getIsNew()==1)||pbc!=null){
			String result = "";
			...
			long t = System.currentTimeMillis()-time1;
			System.out.println("回调商户url:"+notifyUrl+"消耗时间"+t+";接收："+result);
			// 通知商户请求失败
			if (!"SUCCESS".equals(result.toUpperCase())) {
				payv2PayOrderNotifyService.insetNotify(order.getOrderNum(), notifyUrl, param.toString(), t, result);
			...
			long t = System.currentTimeMillis()-time1;
			System.out.println("回调商户url:"+notifyUrl+"消耗时间"+t+";接收http状态："+code);
			// 通知商户请求失败
			if (code != 200) {
				payv2PayOrderNotifyService.insetNotify(order.getOrderNum(), notifyUrl, param.toString(), t, code+"");
				logger.info("=========》通知商户接口访问失败");
				return false;
			} else {
				order.setCallbackTime(new Date());
		...
		if (payOrder != null) {
			if (payOrder.getPayStatus() != PayFinalUtil.PAY_ORDER_SUCCESS && payOrder.getPayStatus() != 5) {
				...
				Payv2BussSupportPayWay pbspw = payv2BussSupportPayWayService.getWayRate(wayList, payOrder.getPayMoney()
						, payOrder.getCompanyId(), payWayId.intValue(),pbc.getIsRandom(),"WAP");
	...
	public Map<String, Object> queryOrderNum(Map<String, Object> map) throws Exception {
		...
		if (payOrder != null) {
			...
			if(payOrder.getAppId()!=null){
				pbca = new Payv2BussCompanyApp();
				pbca.setId(payOrder.getAppId());
				pbca = payv2BussCompanyAppMapper.selectSingle(pbca);
			}else{
				pbc = new Payv2BussCompany();
				pbc.setId(payOrder.getCompanyId());
				payv2BussCompanyMapper.selectSingle(pbc);
			}
			String returnUrl = payOrder.getReturnUrl();
			if(returnUrl!=null&&!"".equals(returnUrl)){
				Map<String, Object> param = new HashMap<>();
				param.put("result_code", ""+PayFinalUtil.SUSSESS_CODE); // 成功
				param.put("pay_discount_money", ""+payOrder.getPayDiscountMoney()); // 最终支付金额
				param.put("pay_time", DateStr(payOrder.getCreateTime())); // 支付时间
				if(payOrder.getPayStatus()==3){
					param.put("result_code", PayFinalUtil.CANCEL_CODE); // 失败
					param.put("pay_money", payOrder.getPayMoney());
					param.put("pay_discount_money", "0.00");
				}
				param.put("order_num", payOrder.getOrderNum()); // 订单
				String sign = "";
				if(payOrder.getAppId()!=null){
					param.put("buss_order_num", payOrder.getMerchantOrderNum()); // 商户订单
					// 参数签名
					sign = PaySignUtil.getSign(param, pbca.getAppSecret());
				}else{
					// 参数签名
					sign = PaySignUtil.getSign(param, pbc.getCompanySecret());
				}
				returnUrl=returnUrl+"?"+PaySignUtil.getParamStr(param)+"&sign="+sign;
			...
			if (payOrder.getPayStatus() != PayFinalUtil.PAY_ORDER_SUCCESS && payOrder.getPayStatus() != 5) {
				...
				if (payOrder.getRateId() != null) {
					...
					//兴业深圳订单查询
							...
							|| PayRateDictValue.PAY_TYPE_XYSZ_QQ_SCAN.equals(ppwr.getDictName())) {
					...
					//商务通订单查询
					else if(PayRateDictValue.PAY_TYPE_SWT_ALI_SCAN.equals(ppwr.getDictName()) 
							|| PayRateDictValue.PAY_TYPE_SWT_WEIXIN_SCAN.equals(ppwr.getDictName())) {
						Map<String, String> queryOrder = SWTPay.queryOrder(ppwr.getRateKey1(), payOrder.getOrderNum(), ppwr.getRateKey2());
						if (queryOrder.containsKey("out_trade_no")) {
							String total_fee = queryOrder.get("total_fee");// 支付金额
							// 订单更新
							payOrder.setPayDiscountMoney(Double.valueOf(total_fee));
					...
					//遂宁订单查询
					else if (PayRateDictValue.PAY_TYPE_SNBANK_ALI_SCAN.equals(ppwr.getDictName()) 
							|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_SCAN.equals(ppwr.getDictName())
							|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_GZH.equals(ppwr.getDictName())
							|| PayRateDictValue.PAY_TYPE_SNBANK_WEIXIN_WAP.equals(ppwr.getDictName())
							|| PayRateDictValue.PAY_TYPE_SNBANK_QQ_SCAN.equals(ppwr.getDictName())) {
						Map<String, String> queryOrder = SNBankPay.queryOrder(payOrder.getOrderNum(), ppwr.getRateKey1(), ppwr.getRateKey2());
						if (queryOrder.containsKey("out_trade_no")) {
							String total_fee = queryOrder.get("total_fee");// 支付金额
							// 订单更新
							payOrder.setPayDiscountMoney(Double.valueOf(total_fee));
					...
					//浦发深圳订单查询
					else if (PayRateDictValue.PAY_TYPE_PFBANK_ALI_SCAN.equals(ppwr.getDictName()) 
							|| PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_SCAN.equals(ppwr.getDictName())
							|| PayRateDictValue.PAY_TYPE_PFBANK_WEIXIN_WEB.equals(ppwr.getDictName())) {
						Map<String, String> queryOrder = PFSZBankPay.queryOrder(payOrder.getOrderNum(), payOrder.getRateKey1(), payOrder.getRateKey2(), 
								new SimpleDateFormat("yyyyMMdd").format(payOrder.getCreateTime()), payOrder.getRateKey5(), payOrder.getRateKey6());
						if("10000".equals(queryOrder.get("code"))){
							String total_fee = DecimalUtil.centsToYuan(queryOrder.get("total_fee"));
							// 订单更新
							payOrder.setPayDiscountMoney(Double.valueOf(total_fee));
							payOrder.setPayStatus(PayFinalUtil.PAY_ORDER_SUCCESS);
							resultMap.put("payOrder", payOrder);
					 ...
					 */
							...
							||ppwr.getDictName().equals(PayRateDictValue.PAY_TYPE_WFT_QQ_SCAN)
							||ppwr.getDictName().equals(PayRateDictValue.PAY_TYPE_WFT_ALI_SCAN)
							||ppwr.getDictName().equals(PayRateDictValue.PAY_TYPE_WFT_WEIXIN_SCAN)) {
					...
					//现在支付订单查询
					else if (PayRateDictValue.PAY_TYPE_NOW_ALI_WAP.equals(ppwr.getDictName())
							||PayRateDictValue.PAY_TYPE_NOW_WEIXIN_WAP.equals(ppwr.getDictName())) {
						Map<String, String> query = NowPayUtil.query(payOrder.getOrderNum(),ppwr.getRateKey1(), ppwr.getRateKey2());
						if ("10000".equals(query.get("code"))) {
							String total_fee = query.get("total_fee");// 支付金额
							// 订单更新
							payOrder.setPayDiscountMoney(Double.valueOf(total_fee));
							payOrder.setPayStatus(PayFinalUtil.PAY_ORDER_SUCCESS);
							resultMap.put("payOrder", payOrder);
					...
					//无卡订单查询
					else if (PayRateDictValue.PAY_TYPE_WUKA_QQ_SCAN.equals(ppwr.getDictName())
							||PayRateDictValue.PAY_TYPE_WUKA_WEIXIN_SCAN.equals(ppwr.getDictName())) {
						Map<String, String> query = WukaPay.query(payOrder.getOrderNum(),ppwr.getRateKey1(), ppwr.getRateKey2());
						if ("10000".equals(query.get("code"))) {
							// 订单更新
							payOrder.setPayDiscountMoney(payOrder.getPayMoney());
							payOrder.setPayStatus(PayFinalUtil.PAY_ORDER_SUCCESS);
					...
					//招商厦门订单查询
					else if (PayRateDictValue.PAY_TYPE_ZSXM_ALI_SCAN.equals(ppwr.getDictName())
							||PayRateDictValue.PAY_TYPE_ZSXM_WEIXIN_SCAN.equals(ppwr.getDictName())) {
						String payType="1";
						if(ppwr.getDictName().contains("_WEIXIN")){
							payType="2";
						}
						Map<String, String> query = ZsxmPay.query(payOrder.getOrderNum(),payType,ppwr.getRateKey1(), ppwr.getRateKey2());
						if ("10000".equals(query.get("code"))) {
							// 订单更新
							payOrder.setPayDiscountMoney(payOrder.getPayMoney());
							payOrder.setPayStatus(PayFinalUtil.PAY_ORDER_SUCCESS);
...
//				String form = AliPay.alipayH5(null, payv2PayOrder.getOrderNum(), payv2PayOrder.getOrderName(), payDiscountMoney);
	...
	public Map<String, String> dyCreateOrder(Map<String, Object> map, Payv2BussCompanyApp pbca
		...
		List<Payv2BussSupportPayWay> payWayList = payv2BussSupportPayWayService
				.queryByCompany(pbca.getCompanyId(), payViewType, dictName,payMoney);
		...
		Payv2BussSupportPayWay pbspw = payv2BussSupportPayWayService.getWayRate(payWayList, payMoney, pbca.getCompanyId()
				, payViewType,pbca.getIsRandom(),dictName);
		...
		resultMap.put("companyKey", pbca.getCompanyKey());
		...
		if(payOrder!=null){
			// 订单重复
			resultMap.put("status", PayFinalUtil.PAY_REPEAT_FAIL);
			return resultMap;
			/*if(payOrder.getPayStatus()!=1&&payOrder.getPayStatus()!=5){
				payOrder.setReturnUrl(map.get("returnUrl") == null ? null : map.get("returnUrl").toString()); // 回调的页面
				payOrder.setNotifyUrl(map.get("notifyUrl").toString()); // 回调地址（服务器）
		...
		resultMap.put("orderName", payOrder.getOrderName());
		resultMap.put("merchantOrderNum", payOrder.getMerchantOrderNum());
	...
	public PageObject<Payv2PayOrder> getPayOrderPageByObject(Map<String, Object> map) {
		Integer totalData = payv2PayOrderMapper.getPayOrderPageByObjectCount(map);
		if(null==totalData){
			totalData=0;
		}
		...
		for (Payv2PayOrder payv2PayOrder : list) {
			...
			payv2PayOrder.setSupportRefund(2);
			if(payv2PayOrder.getPayStatus()==1||payv2PayOrder.getPayStatus()==5||payv2PayOrder.getPayStatus()==6){
				Date date1 = new Date();
				Date date2 = payv2PayOrder.getCreateTime();
				if((date1.getTime()-date2.getTime())/(1000*60*60*24)<=30){
					payv2PayOrder.setSupportRefund(1);
				}
			}
			if(payv2PayOrder.getOrderType()==2){
				payv2PayOrder.setAppName("收款码");
			}
		}
...
//			payv2PayWay.setStatus(1);
	...
	public Map<String, String> payPage(String ip,String appType, Map<String, Object> paramMap
		...
		String dictName = "WAP";//wap支付
		if(payViewType==1){
			dictName = "APP";//app支付
		}
		paramMap.put("address", ip);
		// 创建订单
		Map<String, String> orderMap = dyCreateOrder(paramMap, pbca, payViewType,dictName);
		if (orderMap.get("status").equals(PayFinalUtil.PAY_STATUS_SUSSESS)) {
			dictName="APP";
			return PayRateDictValue.ratePay(orderMap.get("dictName").toString(), orderMap, ip, appType,dictName);
		}
		return orderMap;
	}
	...
	*/
	...
	public List<Map<String,Object>> selectByRate(Map<String,Object> map){
		map.put("createTimeBegin", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 00:00:00");
		map.put("createTimeEnd", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 23:59:59");
		return payv2PayOrderMapper.selectByRate(map);
	}
	
	public List<Map<String,Object>> selectByCom(Map<String,Object> map){
		map.put("createTimeBegin", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 00:00:00");
		map.put("createTimeEnd", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 23:59:59");
		return payv2PayOrderMapper.selectByCom(map);
	}
	
	public List<Map<String,Object>> selectByRateCom(Map<String,Object> map){
		map.put("createTimeBegin", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 00:00:00");
		map.put("createTimeEnd", DateUtil.DateToString(new Date(), DateStyle.YYYY_MM_DD)+" 23:59:59");
		return payv2PayOrderMapper.selectByRateCom(map);
	}
	...
	public Map<String,Object> getDayMoneySum2(Map<String, Object> map) {
		return payv2PayOrderMapper.getDayMoneySum2(map);
	}

	public PageObject<Payv2PayOrder> offlineOrderList(Map<String, Object> map) {
		Integer totalData = payv2PayOrderMapper.offlineOrderListCount(map);
		PageHelper pageHelper = new PageHelper(totalData, map);
		List<Payv2PayOrder> list = payv2PayOrderMapper.offlineOrderList(pageHelper.getMap());
		PageObject<Payv2PayOrder> pageObject = pageHelper.getPageObject();
		pageObject.setDataList(list);
		return pageObject;
	}

	public Double offlineOrderTotalMoney(Map<String, Object> map) {
		Double totalMoney = payv2PayOrderMapper.offlineOrderTotalMoney(map);
		return totalMoney;
	}

	public boolean callBackCom(Payv2PayOrder order, Payv2BussCompanyApp pbca,Payv2BussCompany pbc) throws Exception  {		
		//业务处理
		Map<String, Object> param = new HashMap<>();
		param.put("result_code", ""+PayFinalUtil.SUSSESS_CODE); // 成功
		param.put("order_num", order.getOrderNum()); // 订单
		...
		if((pbca!=null&&pbca.getIsNew()==1)||pbc!=null){
			...
			long t = System.currentTimeMillis()-time1;
			System.out.println("回调商户"+notifyUrl+"消耗时间"+t+";接收http状态："+code);
			// 通知商户请求失败
			if (code != 200) {
				payv2PayOrderNotifyService.insetNotify(order.getOrderNum(), notifyUrl, param.toString(), t, code+"");
				logger.info("=========》通知商户接口访问失败");
