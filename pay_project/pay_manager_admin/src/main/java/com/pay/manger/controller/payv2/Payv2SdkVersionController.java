...
public class Payv2SdkVersionController extends BaseManagerController<Payv2SdkVersion,Payv2SdkVersionMapper> {
    ...
    */
	...
	public Map<String, Object> updatePayv2SdkVersion(@RequestParam Map<String, Object> map,HttpServletRequest request) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		boolean isNotNull = ObjectUtil.checkObjectFile(new String[] { "id","isOnline" }, map);
		if (isNotNull) {
			try {
				payv2SdkVersionService.update(map);
				if(map.get("isOnline").toString().equals("1")) {
					sysLogService.addSysLog(1, LogTypeEunm.T26, IpAddressUtil.getIpAddress(request), getAdmin().getId(), map);
				} else {
					sysLogService.addSysLog(1, LogTypeEunm.T27, IpAddressUtil.getIpAddress(request), getAdmin().getId(), map);
				}
	...
	public Map<String, Object> delPayv2SdkVersion(@RequestParam Map<String, Object> map,HttpServletRequest request) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		boolean isNotNull = ObjectUtil.checkObjectFile(new String[] { "id" }, map);
		if (isNotNull) {
			try {
				payv2SdkVersionService.delete(map);
				sysLogService.addSysLog(1, LogTypeEunm.T24, IpAddressUtil.getIpAddress(request), getAdmin().getId(), map);
				resultMap.put("resultCode", 200);
			} catch (Exception e) {
	...
	public Map<String, Object> addPayv2SdkVersion(@RequestParam Map<String, Object> map,HttpServletRequest request) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		try {
			map.put("createTime", new Date());
			payv2SdkVersionService.add(map);
			sysLogService.addSysLog(1, LogTypeEunm.T23, IpAddressUtil.getIpAddress(request), getAdmin().getId(), map);
			resultMap.put("resultCode", 200);
		} catch (Exception e) {
	...
	public Map<String, Object> editPayv2SdkVersion(@RequestParam Map<String, Object> map, HttpServletRequest request) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		boolean isNotNull = ObjectUtil.checkObjectFile(new String[] { "id" }, map);
		if (isNotNull) {
			try {
				...
				payv2SdkVersionService.update(map);
				sysLogService.addSysLog(1, LogTypeEunm.T25, IpAddressUtil.getIpAddress(request), getAdmin().getId(), map);
				resultMap.put("resultCode", 200);
			} catch (Exception e) {
