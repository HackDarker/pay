...
public class TestExportExcel2<T> {
	...
	public void exportExcel(Map<String,Object> map,Map<Long, Payv2DayCompanyClear> dataMap,Map<Long, List<Payv2DayCompanyClear>> dayCompanyMap,OutputStream out) {
		// 声明一个工作薄
		...
		for (Entry<Long, List<Payv2DayCompanyClear>> entry : dayCompanyMap
				.entrySet()) {
			// 判断是否已经存在该表格，如果不存在，新建一个，如果存在，换个名字新建一个						
			HSSFSheet sheet1 = workbook.createSheet();
			if(sheetNames.contains(entry.getValue().get(0).getCompanyName().toString())){
				workbook.setSheetName(i, entry.getValue().get(0).getCompanyName()+1);
				sheetNames.add(entry.getValue().get(0).getCompanyName());
				i++;				
			}else{
				workbook.setSheetName(i, entry.getValue().get(0).getCompanyName());
				sheetNames.add(entry.getValue().get(0).getCompanyName().toString());
				i++;
			}
			// 设置表格默认列宽度为15个字节
