...
//})
/*编辑*/
$("body").on("click",".type-timeLi li i",function(){
	if($(this).parent().attr("state")==2){
		$(this).css("background-image","url(../../public/img/cs.png)");
		$(this).parent().attr("state","1")
	}else if($(this).parent().attr("state")==1){
		$(this).css("background-image","url(../../public/img/nc.png)");
	...
	if($(".isios").attr("state")==1) {
		$(".platform").show();
		$(".ioss").show();
		$(".iph").show();
		$(".ipd").show();
		$(".iph i").css("background-image","url(../../public/img/nc.png)");
		$(".ipd i").css("background-image","url(../../public/img/nc.png)");
	} else {
		$(".ioss").hide();
		$(".iph").hide();
		$(".ipd").hide();
	...
	if($(".and").attr("state")==1||$(".isios").attr("state")==1){
...
$(".edit").on("click",function(){
	...
	if(ec==1){
		...
		$(".type-timeLi").addClass("chooseType");
		$(".type-timeLi").html('<li class="and fl" state="2"><i></i><span>安卓</span></li><li state="2" class="isios fl"><i></i><span>IOS</span></li><li state="2" class="web fl"><i></i><span>WEB</span></li>')
		$(".account-footer").show();
		$(".imgsCon i").show();
		$(".addImg").show();
		$(".addimg").show();
...
function inidata(data){
	...
	localStorage.isAndroid = data.isAndroid;
	localStorage.isIos = data.isIos;
	localStorage.isWeb = data.isWeb;
	if(data.isAndroid==2){
		$(".android").hide();
		$(".ad-dl").hide();
	}else if(data.isAndroid==1){
		$(".type-timeLi").append($("#andMd").html())
	}
	if(data.isIos==2){
		$(".ioss").hide();
		$(".iph").hide();
		...
		$(".type-timeLi").append($("#iosMd").html())
	}
	if(data.isWeb==1){
		//alert(1)
		//console.log($("#webMd").html())
		$(".type-timeLi").append($("#webMd").html())
		$(".type-timeLi").find(".web").addClass("activess")
	}
	if(data.appImg){
